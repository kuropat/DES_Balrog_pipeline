...
#        print " -m <mof conf file>"
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:],"hc:t:",["confile=","tilename"])
...
#        print " -m <mof conf file>"
    ...
    balP = BalrogBase(confile,tilename)
    balP.prepMeds()
...
#    pool.join()
    ...
    will run them in per band mode. After injection if new source file will be created
    we need to make new file lists for coadd """
    " Now do this for each realization "
    datadir = balP.tiledir
